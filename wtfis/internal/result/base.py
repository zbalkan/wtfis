...
from wtfis.internal.models.shodan import ShodanIp, ShodanIpMap
from wtfis.internal.models.virustotal import LastAnalysisStats, PopularityRanks
...
class BaseResult():
    ...
    def _gen_vt_analysis_stats(
        ...
        # Total count
        total = stats.harmless + stats.malicious + \
            stats.suspicious + stats.timeout + stats.undetected
    ...
    def _gen_vt_response(self) -> dict:
        ...
        baseurl = self.vt_gui_baseurl_ip if is_ip(
            self.entity.data.id_) else self.vt_gui_baseurl_domain
        ...
        vt["updated"] = datetime.fromtimestamp(
            attributes.last_modification_date).isoformat()

        # Last seen (Domain only)
        if hasattr(attributes, "last_dns_records_date"):
            vt["last_seen"] = datetime.fromtimestamp(
                attributes.last_dns_records_date).isoformat()
    ...
    def _gen_ip_enrich_response(self) -> Optional[dict]:
        ...
        if enrich is None:
            ...
            if isinstance(enrich, IpWhois):
                ...
                enrichment["shodan_link"] = f"{self.shodan_gui_baseurl}/{self.entity.data.id_}"
