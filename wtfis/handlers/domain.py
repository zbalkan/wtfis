...
class DomainHandler(BaseHandler):
    ...
    def fetch_data(self) -> None:
        ...
        if self.resolutions and self.resolutions.data:
            ...
            self._fetch_ip_enrichments([rd.attributes.ip_address for rd in self.resolutions.data])

            if self._greynoise:
                print(f"Fetching IP enrichments from {self._greynoise.name}")
                self._fetch_greynoise([rd.attributes.ip_address for rd in self.resolutions.data])
